...
namespace LaTirelire.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    // Ajouter ds le tablo session l'ID du client qui a comme IdUser
                    // la valeur de l'ID du Httpcontxt.User.Identity
                    // ? sauf si (test) un role spécifique existe ds table de role
                    ...
                    if (repRoles.Lister().Where(c => c.Id == userID).Count() == 0)
                    {

                        IRepository<Client> repClient = new EFRepository<Client>();

                        if (repClient.TrouverChaine(userID)!=null)
                        {
                            if (repClient.TrouverChaine(userID).actif)
                            {
                                return RedirectToLocal(returnUrl);
                            }
                            else // sinon le client n'est pas actif : ne pas le logger !
                            {
                                result = SignInStatus.Failure;
                                ViewBag.MessageLogin = "Client désactivé, merci de nous contactez pour plus d'informations";
                                return View();
                            } 
                        }
                        else
        ...
        // GET: /Account/RegisterBO
        [AllowAnonymous]
        public ActionResult RegisterBO()
        {
            RegisterViewModel niouZeur = new RegisterViewModel();
            return View(niouZeur);
        }

        // POST: /Account/RegisterBO
        [HttpPost]
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RegisterBO(RegisterViewModel model) 
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email };
                ...
                if (result.Succeeded)
                {
                    ...
                    return RedirectToAction("IndexBO", "Client");
                }
                AddErrors(result);              
            }
            // Si nous sommes arrivés là, un échec s’est produit. Réafficher le formulaire          
